...
template<template<typename T, long unsigned int N> class Ref, typename T, long unsigned int N>
...
class ContainerIsEqualToMemFactory
{
  public:
    ...
    ContainerIsEqualToMemMatcher<Container, Blas, T>* create(Container&& container, Blas* blas, T delta)
    {
      return new ContainerIsEqualToMemMatcher<Container, Blas, T>(std::forward<Container>(container), blas, delta);
    }
};
...
class MemIsEqualToContainerFactory
{
  public:
    template<typename Container, typename Blas, typename T>
    MemIsEqualToContainerMatcher<Container, Blas, T>* create(Container&& container, Blas* blas, T delta)
    {
      return new MemIsEqualToContainerMatcher<Container, Blas, T>(std::forward<Container>(container), blas, delta);
    }
};
...
Matcher<Mem*> MemIsEqualTo(CT2&& ct2, Blas *blas, T delta) {
  ...
  using CT2_is_Mem_Ptr = std::is_same<typename std::remove_reference<CT2>::type, Mem*>;
  using CT2_is_vector = is_specialization<typename std::remove_reference<CT2>::type, std::vector>;
  using CT2_is_array = is_array_specialization<typename std::remove_reference<CT2>::type, std::array>;

  constexpr bool isMemIsEqualToMemFactory = CT2_is_Mem_Ptr::value;
  constexpr bool isMemIsEqualToContainerFactory = CT2_is_vector::value || CT2_is_array::value;

  using typeMemIsEqualToContainerFactory = typename std::conditional<isMemIsEqualToContainerFactory, MemIsEqualToContainerFactory, std::false_type>::type;
  typename std::conditional<isMemIsEqualToMemFactory, MemIsEqualToMemFactory, typeMemIsEqualToContainerFactory>::type factory;

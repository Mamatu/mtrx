...
#ifndef MTRX_CUBLAS_THREADSMAPPER_HPP
#define MTRX_CUBLAS_THREADSMAPPER_HPP
...
void SetThreadsBlocks(uint blocks[2], uint threads[2], uint w, uint h,
                      uint threadsLimit);
...
template <typename T>
void getThreadsMap(ThreadsMap<T> &threadsMap, T *array, T threadIndex) {
  memcpy(&threadsMap, array + threadIndex * 4, sizeof(ThreadsMap<T>));
}

template <typename T>
void createThreadsMap(T *array, T blocksCount[2], T threadsCount[2], T width,
                      T height) {
...
template <typename T> T getThreadsCount(T threadsCount, T width, T height) {
  T blocksCount[2] = {1, 1};
...
template <typename T>
T createThreadsMap(T *array, T threadsCount, T width, T height) {
...
template <typename T>
void createThreadsMap(T *array, T threadsCount[2], T width, T height) {
  T blocksCount[2] = {1, 1};
  createThreadsMap(array, blocksCount, threadsCount, width, height);
}

template <typename T> T createThreadsMap(T *array, T threadsCount, T length) {
  T a = T(1);

...
#ifndef MTRX_CUBLAS_CUDA_DIAGONAL_ADD_HPP
#define MTRX_CUBLAS_CUDA_DIAGONAL_ADD_HPP
...
__device__ void cuda_SF_diagonalAdd(int /*m*/, int /*n*/, float *matrix,
                                    int lda, float value) {
  HOST_INIT();
  const int y = threadIdx.y + blockDim.y * blockIdx.y;
  const int idx = y * lda + y;
  HOST_CODE(float host_matrixValue = matrix[idx];);
  matrix[idx] = matrix[idx] + value;
  HOST_CODE(logMulResult(__FILE__, __func__, __LINE__, matrix, idx,
                         host_matrixValue, value, y, lda);)
}

__device__ void cuda_SD_diagonalAdd(int /*m*/, int /*n*/, double *matrix,
...
__device__ void cuda_CF_diagonalAdd(int /*m*/, int /*n*/, cuComplex *matrix,
                                    int lda, cuComplex value) {
  HOST_INIT();
  const int y = threadIdx.y + blockDim.y * blockIdx.y;
  const int idx = y * lda + y;
  HOST_CODE(cuComplex host_matrixValue = matrix[idx];);
  matrix[idx] = cuCaddf(matrix[idx], value);
  HOST_CODE(logMulResultComplex(__FILE__, __func__, __LINE__, matrix, idx,
                                host_matrixValue, value, y, lda);)
}

__device__ void cuda_CD_diagonalAdd(int /*m*/, int /*n*/,
  ...
  matrix[idx] = cuCadd(matrix[idx], value);
  HOST_CODE(logMulResultComplex(__FILE__, __func__, __LINE__, matrix, idx,
                                host_matrixValue, value, y, lda);)
}
